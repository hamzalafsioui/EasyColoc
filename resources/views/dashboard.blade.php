        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @php
                    $activeMemberships = auth()->user()->activeMemberships();
                    @endphp
                    ...
                    <div class="mb-8">
                        <div class="flex items-center justify-between mb-6 border-b border-gray-100 dark:border-gray-700 pb-4">
                            <h3 class="text-xl font-bold">Your Active Colocations</h3>
                            <a href="{{ route('colocations.create') }}" class="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">
                                + Create New
                            </a>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div class="flex flex-col p-6 rounded-2xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600 transition-all hover:shadow-md">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <span class="px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest rounded {{ $membership->role === 'owner' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800' }}">
                                        {{ $membership->role }}
                                    </span>
                                </div>
                                <h4 class="text-lg font-bold mb-1 truncate">{{ $membership->colocation->name }}</h4>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 line-clamp-2">
                                    {{ $membership->colocation->description ?? 'No description.' }}
                                </p>
                                <a href="{{ route('colocations.show', $membership->colocation_id) }}" class="mt-auto w-full text-center px-4 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-indigo-700 transition-colors uppercase tracking-widest">
                                    Open Colocation
                                </a>
                            </div>
                            @endforeach
